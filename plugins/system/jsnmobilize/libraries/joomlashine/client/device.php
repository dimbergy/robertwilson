...
/**
 * Class for detecting client device type.
 *
 * @package     JSN_Mobilize
 * @subpackage  SystemPlugin
 * @since       1.0.0
 */
class JSNClientDevice
{
	/**
	* Define data for client device type detection.
	*
	* @var array
	*/
	private static $_types = array(
		...
		// Signatures for detecting tablet PC
		'tablet' => array(
		...
		// Signatures for detecting mobile device
		'mobile' => array(
			// Known HTTP header
			'HTTP_PROFILE'					=> true,
			'HTTP_UA_CPU'					=> 'ARM',
			'HTTP_UA_OS'					=> true,
			'HTTP_WAP_CONNECTION'			=> true,
			'HTTP_X_HUAWEI_USERID'			=> true,
			'HTTP_X_MOBILE_GATEWAY'			=> true,
			'HTTP_X_NOKIA_GATEWAY_ID'		=> true,
			'HTTP_X_NOKIA_IPADDRESS'		=> true,
			'HTTP_X_OPERAMINI_PHONE_UA'		=> true,
			'HTTP_X_ORANGE_ID'				=> true,
			'HTTP_X_VODAFONE_3GPDPCONTEXT'	=> true,
			'HTTP_X_WAP_CLIENTID'			=> true,
			...
			'HTTP_USER_AGENT' => array(
				...
				// Nexus phone
				'Nexus One|Nexus S|Galaxy.*Nexus|Android.*Nexus',

				// Palm phone
				'PalmSource|Palm|blazer|centro|elaine|eudoraweb|hiptop|plink|plucker|pre/|treo|xiino',
				...
				// Sony phone
				'E10i|SonyEricsson|SonyEricssonLT15iv',
				...
				'\bBada\b',																				// Bada
				'(blackberry|rim tablet os)',															// BlackBerry
				'(iphone|ipod|ipad)',																	// iOS
				'(avantgo|blazer|elaine|hiptop|palm|plucker|xiino)',									// Palm
				'Symbian|SymbOS|Series60|Series40|\bS60\b',												// Symbian
				'IEMobile|Windows Phone|Windows CE.*(PPC|Smartphone)|MSIEMobile|Window Mobile|XBLWP7',	// Windows Mobile

				// Mobile web browser
				'Blazer',															// Blazer
				'bolt',																// Bolt
				'\bCrMo\b|Chrome\/[.0-9]* Mobile',									// Chrome Mobile
				'\bDolfin\b',														// Dolfin
				'fennec|firefox.*maemo|(Mobile|Tablet).*Firefox|Firefox.*Mobile',	// Firefox Mobile
				'IEMobile|MSIEMobile',												// IE Mobile
				'midori',															// Midori
				'Opera.*Mini|Opera.*Mobi|Android.*Opera',							// Opera Mobile/Mini
				'Mobile.*Safari|Safari.*Mobile',									// Safari Mobile
				'skyfire',															// Skyfire
				'teashark',															// TeaShark
				'NokiaBrowser|OviBrowser|SEMC.*Browser'								// Generic browser
			)
		)
	);

	/**
	* Method to detect client device type.
	*
	* @param   array  $httpHeader  Array of HTTP header server received from client.
	*
	* @return  string  Client device type: desktop, tablet or mobile
	*/
	public static function detect($httpHeader = null)
	{
		if (empty($httpHeader) OR ! is_array($httpHeader) OR ! count($httpHeader))
		{
			$httpHeader = & $_SERVER;
		}

		// Preset client device type
		$detected = 'desktop';

		// Loop thru declared signatures for detecting client device type
		foreach (self::$_types AS $device => $data)
		{
			foreach ($data AS $header => $signature)
			{
				// Check if a header is set
				if (is_bool($signature) AND $signature AND isset($httpHeader[$header]))
				{
					$detected = $device;
					break 2;
				}
				// Check if a header is set and equal to signature
				elseif (is_string($signature) AND isset($httpHeader[$header]) AND $httpHeader[$header] == $signature)
				{
					$detected = $device;
					break 2;
				}
				// Check if user agent match signature
				elseif (is_array($signature))
				{
					// Get server variable
					if ($header == 'HTTP_USER_AGENT')
					{
						$versus	= isset($httpHeader['HTTP_USER_AGENT']) ? $httpHeader['HTTP_USER_AGENT']
								: (isset($httpHeader['HTTP_X_DEVICE_USER_AGENT']) ? $httpHeader['HTTP_X_DEVICE_USER_AGENT'] : '');
					}
					else
					{
						$versus = isset($httpHeader[$header]) ? $httpHeader[$header] : '';
					}
					...
					if ( ! empty($versus))
					{
						...
						{
							if ($header == 'HTTP_USER_AGENT' AND preg_match("#{$pattern}#i", $versus))
							{
								$detected = $device;
								break 3;
							}
							elseif (strpos($versus, $pattern) !== false)
							{
								$detected = $device;
								break 3;
							}
