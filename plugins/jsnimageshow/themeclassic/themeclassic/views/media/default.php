 ...
 * @version $Id: default.php 6411 2011-05-27 07:09:16Z trungnq $
...
<?php }?>
<?php if($act =='pattern'){?>
var image_base_path = 'plugins/<?php echo $this->_showcaseThemeType.'/'.$this->_showcaseThemeName.'/'.$this->_showcaseThemeName; ?>/assets/images/bg-patterns/';
window.addEvent('domready', function(){
	var str = window.parent.$('imgpanel_bg_value_first').value;	
	if(str.indexOf("bg-patterns") != -1 && str.indexOf("bg-images") == -1){
		$('f_url').value = window.parent.$('imgpanel_bg_value_first').value;	
...
<form action="index.php" id="imageForm" method="post" enctype="multipart/form-data">
    ...
    <div class="button">
	<button type="button" onclick="JSNISImageManager.onok('<?php echo $act?>');window.top.setTimeout('SqueezeBox.close()', 200);"><?php echo JText::_('Select') ?></button>
	<button type="button" onclick="window.top.setTimeout('SqueezeBox.close()', 200);"><?php echo JText::_('Cancel') ?></button>
	</div>
