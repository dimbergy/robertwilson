	...
	class IG_Pb_Helper_Html {
		...
		static function tiny_mce($element) {
			...
			elseif($jCfg->get('editor') == 'jce')
			{
                ...
                $fontSizes = $wf->getParam('fontsizeselect.font_sizes', '8pt,10pt,12pt,14pt,18pt,24pt,36pt', '8pt,10pt,12pt,14pt,18pt,24pt,36pt');
                if ($fontSizes == '')
                {
                ...
                $jceData .= " data-font_sizes=" . $fontSizes;
		...
		static function radio_button_group($element) {
			...
			foreach ($element['options'] as $key => $text) {
				$checked = ($key == $element['std']) ? 'checked="checked"' : '';
