(function($) {
	...
	if (typeof $.JSNEasySliderModal != 'undefined') {
		return;
	}

	$.JSNEasySliderModal = function(options) {
		this.options = $.extend({
			iframe: true,
			frameId: '',
			jParent : $,
			dialogClass: 'jsn-es-dialog',
			url: 'about:blank',
			scrollable: false,
			width: 500,
			height: 300,
		...
		this.overlay = this.options.jParent('.jsn-es-modal-overlay');

		if (this.overlay.size() == 0) {
			this.overlay = $('<div/>', { 'class': 'jsn-es-modal-overlay' });
		}

		this.indicator = this.options.jParent('.jsn-es-modal-indicator');

		if (this.indicator.size() == 0) {
			this.indicator = $('<div/>', { 'class': 'jsn-es-modal-indicator' });
		}

		this.container = this.options.jParent('<div/>', { 'class': 'jsn-es-modal' });
		...
		// Initial modal box
		this.dialog = this.container.dialog($.extend({}, this.options));

		// Register all needed events
		this.registerEvents();
	};

	$.JSNEasySliderModal.prototype = {
		/**
		...
		show: function (callback) {
			...
			this.indicator.css('z-index', this.dialog.zIndex() + 10).show();
		...
		navigateTo: function (url, callback) {
			...
			this.container.parent().addClass('jsn-es-loading');
		...
		onIFrameLoaded: function (e) {
			...
			modal.container.parent().removeClass('jsn-es-loading');
