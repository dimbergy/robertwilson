				...
				return {
					...
					"{startButton} click" : function( element )
					{
						...
						.done(function( files ){
							...
							{
								// Show message that there are no files found.
								// $( '<li>' )
								// 	.html( 'No files with .points found in the site' )
								// 	.appendTo( $( '.scannedResult' ) );
							}
						});
					},
	...
	EasySocial.module("locations/popbox", function($){
		...
		this.resolve(function(popbox){
			...
			return {
				...
				type: "location",
				position: "bottom",
				content: '<img src="' + url + '" width="400" height="200" />'
			}
		});
				...
				return {
					...
					"{acceptFriend} click" : function( el , event )
					{
						...
						.done(function( title , mutualFriendsContent )
						{
							...
							self.actionsWrapper().removeClass( 'friend-adding' ).addClass( 'friend-added' );
		...
		EasySocial.Controller(
			...
			function(self){
				return { 
					...
					{
						// Stop propagation here so it wouldn't trigger the body click events.
						event.stopPropagation();
						...
						if( el.hasClass('open' ) )
						{
							...
							// Trigger closing of drop down item.
							el.trigger( 'hideDropdown' );

							self.item().trigger( 'hideDropdown' );
						}
						else
						{
							self.item().removeClass( 'open' );
