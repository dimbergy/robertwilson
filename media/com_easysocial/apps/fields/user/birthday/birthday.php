...
class SocialFieldsUserBirthday extends SocialFieldItem
{
	...
	public function onRegister( &$post, &$registration )
	{
		...
		{
			$data = $this->getBirthdayValue( $post[ $this->inputName ] );
		}
	...
	public function onDisplay( $user )
	{
		...
		if( $this->params->get( 'year_privacy' ) )
		{
			...
			$allowYear = $lib->validate( 'core.view', $this->field->id, 'field.birthday.year', $user->id );
		}
	...
	private function getBirthdayValue( $data )
	{
	...
	public function validateBirthday( &$post )
	{
		...
		$data	= $this->getBirthdayValue( $value );
	...
	private function saveBirthday( &$post )
	{
		$value 		= isset( $post[ $this->inputName ] ) ? $post[ $this->inputName ] : '';
