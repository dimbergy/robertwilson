...
class SocialFieldsUserJoomla_Username extends SocialFieldItem
{
	...
	public function onRegisterValidate( &$post )
	{
		...
		// If settings is set to use email as username, then we manually add it to the username key
		if( $config->get( 'registrations.emailasusername' ) )
		{
			if( empty( $post['email'] ) )
			{
				return $this->setError( JText::_( 'PLG_FIELDS_JOOMLA_USERNAME_EMAIL_REQUIRED' ) );
			}
