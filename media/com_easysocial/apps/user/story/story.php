...
class SocialUserAppStory extends SocialAppItem
{
	...
	public function onAfterLikeSave( &$likes )
	{
		...
		if( $recipients )
		{
			...
			foreach( $recipients as $recipientId )
			{
				...
				$mailParams[ 'permalink' ]		= FRoute::stream( array( 'id' => $stream->id , 'layout' => 'item', 'external' => true ) );
	...
	public function onAfterCommentSave( &$comment )
	{
		...
		if( $element == SOCIAL_TYPE_STORY )
		{
			...
			foreach( $recipients as $recipient )
			{
				...
				$sysTitle = $recipient == $stream->actor_id ? JText::sprintf( 'COM_EASYSOCIAL_SYSTEM_COMMENTS_ITEM_SYSTEM_TITLE' , $poster->getName() ) : JText::sprintf( 'COM_EASYSOCIAL_SYSTEM_COMMENTS_INVOVLED_TITLE' , $poster->getName() );
				...
				$state 	= Foundry::notify( $rule , array( $recipient ),
											...
											array( 'title' => $sysTitle , 'context_type' => $element , 'url' => $stream->getPermalink( false ), 'actor_id' => $comment->created_by , 'uid' => $comment->uid , 'aggregate' => true )
										);
			}
		}
	...
	public function processLinksNotifications( &$item )
	{
		...
		{
			$item->title 	= JText::_( 'APP_STORY_POSTED_LINK_TAGGED' );
		}
		else
		{
